...
def merge_brute(intervals):
    changed = True
    while changed:           # keep looping until no merges happen
        changed = False
        result = []
        skip = set()         # track which intervals were absorbed

        for i in range(len(intervals)):
            if i in skip:
                continue
            current = intervals[i][:]   # copy to avoid mutation
            ...
            for j in range(i + 1, len(intervals)):
                ...
                if current[0] <= intervals[j][1] and intervals[j][0] <= current[1]:
                    current[0] = min(current[0], intervals[j][0])  # expand
                    current[1] = max(current[1], intervals[j][1])
                    skip.add(j)       # mark j as merged into i
                    changed = True
...
def merge(intervals):
    ...
    merged = [intervals[0][:]]   # start with the first interval (copy it)

    for start, end in intervals[1:]:    # walk through the rest
        last_end = merged[-1][1]        # end time of last merged interval

        if start <= last_end:           # current starts before last ends → OVERLAP
...
def insert_brute(intervals, newInterval):
    intervals.append(newInterval)              # just toss the new interval in
    ...
    for start, end in intervals[1:]:
        if start <= merged[-1][1]:
            ...
            merged.append([start, end])               # no overlap → new
...
def insert(intervals, newInterval):
    ...
    while i < n and intervals[i][1] < newInterval[0]:
        result.append(intervals[i])   # this interval is entirely to the left
        i += 1

    # Phase 2: merge all intervals that OVERLAP with newInterval
    while i < n and intervals[i][0] <= newInterval[1]:
        # current interval overlaps (starts before newInterval ends)
        newInterval[0] = min(newInterval[0], intervals[i][0])  # expand start
        newInterval[1] = max(newInterval[1], intervals[i][1])  # expand end
        i += 1
    result.append(newInterval)   # add the (possibly expanded) merged interval
...
const MI_Q3_BRUTE = `# Brute: check every pair from firstList and secondList
def interval_intersection_brute(firstList, secondList):
    result = []

    for a_start, a_end in firstList:       # every interval in A
        for b_start, b_end in secondList:  # every interval in B
...
def interval_intersection(firstList, secondList):
    result = []
    i = 0   # pointer into firstList
    j = 0   # pointer into secondList
    ...
    while i < len(firstList) and j < len(secondList):
        ...
        start = max(a_start, b_start)   # latest start = intersection start
        end   = min(a_end, b_end)       # earliest end  = intersection end
    ...
    # i=0,j=0: [0,2]∩[1,5]=[1,2] ✅  a_end=2 < b_end=5 → i++
    # i=1,j=0: [5,10]∩[1,5]=[5,5] ✅ b_end=5 < a_end=10 → j++
...
def min_meeting_rooms_brute(intervals):
    ...
    rooms = []   # each entry = end time of that room's current meeting

    for start, end in intervals:
        # try to find a room that's already free
        found = False
        for i in range(len(rooms)):
            if rooms[i] <= start:   # room i finished by the time this meeting starts
                rooms[i] = end      # reuse the room for this meeting
                found = True
                break

        if not found:
            rooms.append(end)   # no free room available → open a new one

    return len(rooms)   # total rooms opened = minimum needed
...
def min_meeting_rooms(intervals):
    ...
    rooms = 0        # rooms currently in use
    max_rooms = 0    # peak rooms needed at any moment
    end_ptr = 0      # pointer into ends — tracks earliest available room
    ...
    for start in starts:               # process each meeting in order
        ...
        max_rooms = max(max_rooms, rooms)  # track peak usage
...
def employee_free_time_brute(schedule):
    ...
    for employee in schedule:
        for interval in employee:        # each interval is [start, end]
            all_ivs.append(interval)

    if not all_ivs:
        return []

    # find the time boundaries
    t_start = min(iv[0] for iv in all_ivs)
    t_end   = max(iv[1] for iv in all_ivs)

    # mark every minute that someone is working
    busy = [False] * (t_end + 1)
    for s, e in all_ivs:
        for t in range(s, e):    # mark working minutes
            busy[t] = True
    ...
    i = t_start
    while i < t_end:
        if not busy[i]:          # found a free minute
            j = i
            while j < t_end and not busy[j]:
                j += 1           # extend the free block
            free.append([i, j])  # free interval [i, j]
            i = j
        else:
            i += 1
...
def employee_free_time(schedule):
    ...
    for employee in schedule:
        for iv in employee:
            all_ivs.append([iv[0], iv[1]])   # copy each [start, end]

    # Step 2: sort all intervals by start time
    all_ivs.sort(key=lambda x: x[0])

    # Step 3: merge overlapping working intervals (same as Q1)
    merged = [all_ivs[0]]
    for start, end in all_ivs[1:]:
    ...
    for i in range(1, len(merged)):
        gap_start = merged[i-1][1]   # previous interval's end
        gap_end   = merged[i][0]     # current interval's start
...
const MI_P1_CODE = `def can_attend_meetings(intervals):
    if not intervals:
        return True
...
const MI_P2_CODE = `def find_min_arrow_shots(points):
    if not points:
        return 0

    points.sort(key=lambda x: x[1])   # sort by END position (right boundary)

    arrows = 1                         # always need at least one arrow
    arrow_pos = points[0][1]           # shoot at the rightmost edge of first balloon

    for start, end in points[1:]:
        if start > arrow_pos:          # this balloon starts after the current arrow
            arrows += 1                # need a new arrow
            arrow_pos = end            # aim at the rightmost edge of this balloon
...
const MI_P3_CODE = `def erase_overlap_intervals(intervals):
    if not intervals:
        return 0
    ...
    last_end = intervals[0][1]            # end of the last interval we kept
...
const MI_P4_CODE = `def car_pooling(trips, capacity):
    ...
    for num_passengers, from_loc, to_loc in trips:
        events.append((from_loc, num_passengers))    # boarding
        events.append((to_loc, -num_passengers))     # leaving

    events.sort()   # sort by location (dropoffs before pickups at same spot)

    current = 0    # passengers currently in the car
    for location, change in events:
        current += change
        if current > capacity:   # at this location, we're over capacity
            return False

    return True   # never exceeded capacity at any point
    # Time: O(n log n), Space: O(n)`

const MI_P5_CODE = `def partition_labels(s):
    # find the last index (position) where each character appears
    last = {c: i for i, c in enumerate(s)}
    # e.g., for "ababcbacadef": a→8, b→5, c→7, d→9, e→10, f→11

    partitions = []   # sizes of each partition
    start = 0         # start index of the current partition
    end = 0           # farthest right we must extend the current partition

    for i, c in enumerate(s):
        end = max(end, last[c])   # must include the last occurrence of c

        if i == end:              # we've covered all occurrences up to 'end'
            partitions.append(end - start + 1)  # record this partition's size
  ...
  return (
    <div className="found-content">
      ...
      <Sub num="★" title="What is Merge Intervals?">
        ...
        <CodeBlock code={MI_VISUAL} lang="python" />
      </Sub>
      ...
      <Sub num="②" title="Practice Questions — Try These Yourself">
        ...
        <QuestionCard
          ...
          bruteCode={`# O(n²): try all possible shot positions\ndef min_arrows_brute(points):\n    xs = set(x for s,e in points for x in range(s,e+1))\n    # (impractical for large ranges — just conceptual)\n    best = len(points)   # worst case: one arrow per balloon\n    # instead: sort + greedy is the standard approach\n    return best`}
          optCode={MI_P2_CODE}
          lang="python"
        />
        ...
        <QuestionCard
          ...
          bruteCode={`# O(n²): simulate at each unique location\ndef car_pooling_brute(trips, capacity):\n    locs = sorted(set(x for _,s,e in trips for x in [s,e]))\n    for loc in locs:\n        passengers = sum(n for n,s,e in trips if s <= loc < e)\n        if passengers > capacity:\n            return False\n    return True`}
          optCode={MI_P4_CODE}
          lang="python"
        />
        ...
        <QuestionCard
          ...
          bruteCode={`# O(n²): try each possible partition point\ndef partition_labels_brute(s):\n    # for each potential cut point, check if characters don't reappear\n    result = []\n    start = 0\n    while start < len(s):\n        end = start\n        for i in range(start, len(s)):\n            # extend end if any char in s[start..i] appears after i\n            for c in set(s[start:i+1]):\n                last = s.rfind(c)\n                end = max(end, last)\n            if end == i:\n                result.append(i - start + 1)\n                start = i + 1\n                break\n    return result`}
          optCode={MI_P5_CODE}
          lang="python"
        />
